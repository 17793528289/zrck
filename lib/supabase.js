        ...
        try {
            ...
            if (window.supabaseReady && window.supabase) {
                try {
                    ...
                    console.warn('API请求失败，使用模拟数据:', apiError.message);
                    throw new Error('账号不存在');
                }
            } else {
                throw new Error('账号不存在');
            ...
            console.warn('获取邮箱失败:', error.message);
            // 检查是否是邮箱格式
            const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
            if (emailRegex.test(account)) {
                // 如果是邮箱格式，直接返回
    ...
    async addUser(userData) {
        try {
            if (window.supabaseReady && window.supabase) {
                ...
                return { data: [newUser], error: null, source: 'mock' };
            }
        } catch (error) {
    ...
    async addActivity(activityData) {
        try {
            if (window.supabaseReady && window.supabase) {
                ...
                return { data: [newActivity], error: null, source: 'mock' };
            }
        } catch (error) {
    ...
    async addResource(resourceData) {
        try {
            if (window.supabaseReady && window.supabase) {
                ...
                mockData.resources.unshift(newResource);
                return { data: [newResource], error: null, source: 'mock' };
            }
        } catch (error) {
    ...
    async addProject(projectData) {
        try {
            if (window.supabaseReady && window.supabase) {
                ...
                return { data: [newProject], error: null, source: 'mock' };
            }
        } catch (error) {
