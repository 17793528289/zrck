    ...
    try {
        ...
        script.onload = function() {
            if (typeof supabase !== 'undefined' && typeof supabase.createClient === 'function') {
                try {
                    const realSupabase = supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
                    if (realSupabase.auth && realSupabase.auth.signInWithPassword) {
                        window.supabase = realSupabase;
                        console.log('Supabase客户端从CDN加载成功');
                    }
                } catch (e) {
                    console.error('初始化真实Supabase客户端失败:', e);
                }
            } else {
